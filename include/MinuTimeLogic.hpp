...
class MinuTimeLogic : public TimeLogic
{
public:
    [[nodiscard]] int64_t isTemporalSensitized(const uint32_t alpha,const uint32_t beta,const std::chrono::high_resolution_clock::time_point &timestamp) override{
        ...
        auto scaled_time = std::chrono::duration_cast<std::chrono::minutes>(now-timestamp).count();
        if((alpha<=scaled_time) && (scaled_time<=beta)){
            return 0;
        } else {
            return std::chrono::duration_cast<std::chrono::minutes>(timestamp - now).count() + alpha;
        }
    }
