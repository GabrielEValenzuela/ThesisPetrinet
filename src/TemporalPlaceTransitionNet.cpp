#include "../include/TemporalPlaceTransitionNet.hpp"

void TemporalPlaceTransitionNet::setFile(std::unique_ptr<ReadFile> load_file) {
...
void TemporalPlaceTransitionNet::buildStepPTMatrix(){
    map_immediate_transitions = std::make_unique<std::map<uint32_t, std::pair<uint8_t, uint8_t>>>();
    z_matrix = std::make_unique<std::vector<std::pair<uint32_t, uint32_t>>>();
...
void TemporalPlaceTransitionNet::auxTransitionsDisc() {
...
void TemporalPlaceTransitionNet::buildStepPTMark(){
...
void TemporalPlaceTransitionNet::buildStepSensitized() {
...
void TemporalPlaceTransitionNet::auxStepSensitized(PetriMatrix<int32_t>* pre_incidence, std::vector<uint32_t>& mark) {
...
void TemporalPlaceTransitionNet::auxStepLVector(std::vector<uint8_t>* l_vec)
{
    ...
    for (auto transition = 0; transition < file->getNumberTransitions(); transition++) {
        ...
        l_vec->at(transition) = std::any_of(aux_vec->begin(), aux_vec->end(), [](bool b) {return b; });
    }
}

void TemporalPlaceTransitionNet::auxStepBVector(std::vector<uint8_t>* b_vec) {
    ...
    for (auto transition = 0; transition < file->getNumberTransitions(); transition++) {
        ...
        b_vec->at(transition) = std::any_of(aux_vec->begin(), aux_vec->end(), [](bool b) {return b; });
    }
}

void TemporalPlaceTransitionNet::buildStepHMatrix()
{
...
void TemporalPlaceTransitionNet::buildStepWMatrix()
{
...
void TemporalPlaceTransitionNet::buildStepAVector()
{
    reset_vector = std::make_unique<std::vector<uint8_t>>(numberTransitions);
    file->readArcsReset(reset_vector.get());
}

void TemporalPlaceTransitionNet::buildStepGuardAndEvent() {
...
std::shared_ptr<PetriNetwork> TemporalPlaceTransitionNet::getPetrinet() {
    setDependencyInjection();
    return petri_instance;
}

void TemporalPlaceTransitionNet::setDependencyInjection() {
    ...
    petri_instance->setTemporalWindow(std::move(z_matrix));
}